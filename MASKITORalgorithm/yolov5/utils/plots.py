...
#ANU: Added the cropping function to get face only    
def copy_detected_face_only(x, img):
    # Copies only the face section from the passed image and detected coordinates
    width = int(x[2]) - int(x[0])
    height = int(x[3]) - int(x[1])
    return (img[int(x[1]):int(x[3]), int(x[0]):int(x[2])])
